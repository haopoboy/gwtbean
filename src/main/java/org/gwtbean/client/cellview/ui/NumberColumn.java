 ...
 *
 */
public class NumberColumn<T extends BeanObject> extends AbstractColumn<T> {
	
	@UiConstructor
	public NumberColumn(String propertyPath) {
		super(propertyPath);
	}

	@Override
	protected Column<T, ?> createColumn() {
		return ColumnFactory.createColumn( CellType.NUMBER, getPropertyPath() ) ;
	}
}
